...
export type TUserProfileData = {
	...
	href?: string;
	/*
	 * display a button to edit the profile only if user has edit rights
	 */
	canEdit: boolean;
};
...
const changeAvatarImage = (): void => {
	console.info('changeAvatarImage: function not implemented.');
};

export const UserProfile: FC<TUserProfile> = ({ size = 'M', border, href, userName, avatar, buttonLabel, canEdit }) => {
		...
		if (size !== 'XL') {
			return (
				<div className={[...baseStyle, 'overflow-hidden', sizeMap[size]].join(' ')}>
					<NextLink
						href={href}
						// For NextLink className should be ok to use
